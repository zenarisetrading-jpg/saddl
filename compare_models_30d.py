...
def compare_models():
    ...
    if not os.path.exists('ml_recs_30d.csv'):
        print("ml_recs_30d.csv not found. Please run ml_bid_optimizer.py first.")
        return
        
    ml_recs = pd.read_csv('ml_recs_30d.csv')
    if ml_recs.empty:
        print("ml_recs_30d.csv is empty.")
        return
    ...
    comparison.to_csv('model_comparison_30d.csv', index=False)
    print("\\nDetailed comparison saved to model_comparison_30d.csv")
