...
- "Realized Impact" = What ACTUALLY happened (Past/Proven). If available, ALWAYS CITE IT to prove value.
...
"""
    ...
    def _compute_optimization_impact(self, df: pd.DataFrame) -> Dict[str, Any]:
        """
        Calculate the financial impact of implementing all optimization recommendations.
        
        Returns:
        - Spend savings from negatives (monthly)
        - Potential sales increase from harvests
        - Bid optimization impact (increases vs decreases)
        - Net financial impact summary
        """
        opt_res = st.session_state.get('optimizer_results') or st.session_state.get('latest_optimizer_run')
        
        if not opt_res:
            return {"status": "Optimizer not run yet - run analysis to see impact projections"}
        
        # Detect date range for monthly projection
        date_info = self._detect_date_range(df)
        weeks = date_info.get('weeks', 1)
        monthly_multiplier = 4.33 / weeks  # Convert to monthly
        ...
        # =====================================================================
        # 1. NEGATIVE IMPACT - Spend that will be eliminated
        # =====================================================================
        neg_kw = opt_res.get('neg_kw', pd.DataFrame())
        neg_pt = opt_res.get('neg_pt', pd.DataFrame())
