...
DEFAULT_CONFIG = {
    ...
    "HARVEST_ORDERS": 3,
    "HARVEST_SALES": 150.0,
    "HARVEST_ROAS_MULT": 1.0,  # vs campaign median
...
def prepare_data(df: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, dict]:
    ...
    df["Targeting"] = df["Targeting"].fillna(df.get("Customer Search Term", "")).fillna("").astype(str)
...
def identify_harvest_candidates(
    ...
    harvest_mask = (
        ...
        (merged["Orders"] >= config["HARVEST_ORDERS"]) &
        (merged["Sales"] >= config["HARVEST_SALES"]) &
        (merged["ROAS"] >= baseline_roas * config["HARVEST_ROAS_MULT"])
    )
...
def calculate_bid_optimizations(
    df: pd.DataFrame, 
    config: dict, 
    harvested_terms: Set[str]
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Calculate optimal bid adjustments using ROAS-based rule engine.
    
    Segments:
    - Direct: Exact match keywords and specific ASIN Product Targets (bid at keyword level)
    - Aggregated: Broad/Phrase/Auto/Category matches (aggregate back to match type)
    
    NEVER optimizes at customer search term level for non-exact.
    """
    # Exclude harvested terms
    df_opt = df[~df["Customer Search Term"].str.lower().isin(harvested_terms)].copy()
    
    # Segment: Direct (Exact keywords + specific ASIN PTs)
    direct_mask = (
        (df_opt["Match Type"].str.contains("exact", case=False, na=False)) |
        (
            df_opt["Targeting"].str.contains(r"asin=|b0[a-z0-9]{8}", case=False, regex=True, na=False) &
            ~df_opt["Targeting"].str.contains(r"category|close-match|loose-match|substitutes|complements", case=False, na=False)
        )
    )
    
    direct_bids = _process_direct_segment(df_opt[direct_mask], config)
    agg_bids = _process_aggregated_segment(df_opt[~direct_mask], config)
    
    return direct_bids, agg_bids


def _process_direct_segment(segment_df: pd.DataFrame, config: dict) -> pd.DataFrame:
    """Process direct segment - bid at keyword/PT level."""
    if segment_df.empty:
        return pd.DataFrame()
    
    segment_df = segment_df.copy()
    
    # Group by Keyword/Target (not search term)
    # Helper to handle NaNs/empty IDs robustly
    def get_group_key(r):
        kid = r.get("KeywordId")
        if pd.notna(kid) and str(kid).strip() != "": return str(kid)
        
        tid = r.get("TargetingId")
        if pd.notna(tid) and str(tid).strip() != "": return str(tid)
        
        return f"{r['Campaign Name']}|{r['Ad Group Name']}|{r['Targeting']}|{r['Match Type']}"

    segment_df["Group_Key"] = segment_df.apply(get_group_key, axis=1)
    
    agg_cols = {
        "Impressions": "sum", "Clicks": "sum", "Spend": "sum",
        "Sales_Attributed": "sum", "Orders_Attributed": "sum", 
        "CPC": "mean", "Campaign_ROAS": "mean"
    }
    
    # Add Current Bid to aggregation if present
    if "Current Bid" in segment_df.columns:
        agg_cols["Current Bid"] = "max"  # Use max for direct segments (should be unique per ID)
    meta_cols = {c: "first" for c in [
        "Campaign Name", "Ad Group Name", "Targeting", "Customer Search Term",
        "CampaignId", "AdGroupId", "Match Type", "KeywordId", "TargetingId"
    ] if c in segment_df.columns}
    
    grouped = segment_df.groupby("Group_Key", as_index=False).agg({**agg_cols, **meta_cols})
    grouped["ROAS"] = np.where(grouped["Spend"] > 0, grouped["Sales_Attributed"] / grouped["Spend"], 0)
    
    # Filter for minimum data
    candidates = grouped[grouped["Clicks"] >= config["MIN_CLICKS_BID"]].copy()
    
    if candidates.empty:
        return pd.DataFrame()
    
    # Apply bid optimization
    alpha = config["ALPHA_EXACT"]
    results = candidates.apply(lambda r: _optimize_bid(r, config, alpha), axis=1)
    candidates["New Bid"] = results.apply(lambda x: x[0])
    candidates["Reason"] = results.apply(lambda x: x[1])
    
    # Rename for compatibility
    candidates["Cost Per Click (CPC)"] = candidates["CPC"]
    
    return candidates


def _process_aggregated_segment(segment_df: pd.DataFrame, config: dict) -> pd.DataFrame:
    """
    Process aggregated segment - bid at match type level.
    Aggregates broad/phrase/category/auto back to the parent targeting.
    """
    if segment_df.empty:
        return pd.DataFrame()
    ...
    # Determine match type category
    def get_match_category(row):
        targeting = str(row.get("Targeting", "")).lower()
        match_type = str(row.get("Match Type", "")).lower()
        
        if "close-match" in targeting:
            return "close-match"
        elif "loose-match" in targeting:
            return "loose-match"
        elif "substitutes" in targeting:
            return "substitutes"
        elif "complements" in targeting:
            return "complements"
    ...
    segment_df["Match_Category"] = segment_df.apply(get_match_category, axis=1)
    
    # Group by Campaign > Ad Group > Target (for Auto/Category) OR Match Type (for broad/phrase)
    # User Request: Aggregate Auto/Category back to "product targeting expression" field value
    def get_group_key(row):
        base = f"{row['Campaign Name']}|{row['Ad Group Name']}"
        cat = row["Match_Category"]
        
        # If it's a specific target type (Auto segment or Category), group by the exact targeting expression
        if cat in ["close-match", "loose-match", "substitutes", "complements", "category"]:
            return f"{base}|{row['Targeting']}"
        # For Broad/Phrase, we aggregate all terms to the match type (Ad Group level performance)
        else:
            return f"{base}|{cat}"
    ...
    # Add Current Bid to aggregation if present
    if "Current Bid" in segment_df.columns:
        agg_cols["Current Bid"] = "mean"  # Use mean for aggregated segments

    meta_cols = {c: "first" for c in [
        "Campaign Name", "Ad Group Name", "Match_Category",
        "CampaignId", "AdGroupId", "KeywordId", "TargetingId", "Targeting"
    ] if c in segment_df.columns}
    
    grouped = segment_df.groupby("Group_Key", as_index=False).agg({**agg_cols, **meta_cols})
    grouped["ROAS"] = np.where(grouped["Spend"] > 0, grouped["Sales_Attributed"] / grouped["Spend"], 0)
    grouped["Match Type"] = grouped["Match_Category"]
    
    # Display Logic:
    # For Broad/Phrase, aggregate to the category name (e.g., "broad")
    # For Granular types (close-match, category=...), keep the specific Targeting expression
    granular_types = ["close-match", "loose-match", "substitutes", "complements", "category"]
    
    grouped["Targeting"] = np.where(
        grouped["Match_Category"].isin(granular_types),
        grouped["Targeting"], # Keep specific target (e.g. category="123")
        grouped["Match_Category"] # Use generic label for others
    )
    ...
    # Apply bid optimization with lower alpha
    alpha = config["ALPHA_BROAD"]
    results = candidates.apply(lambda r: _optimize_bid(r, config, alpha), axis=1)
    candidates["New Bid"] = results.apply(lambda x: x[0])
    candidates["Reason"] = results.apply(lambda x: x[1])
    ...
    # CRITICAL FIX: Re-join IDs after aggregation
    # The IDs from "first" aggregation don't match the aggregated targeting
    # We need to look up IDs based on Campaign + AdGroup + final Targeting value
    from core.data_hub import DataHub
    hub = DataHub()
    bulk = hub.get_data('bulk_id_mapping')
    ...
    if bulk is not None and not bulk.empty:
        # Normalize for matching
        candidates['_camp_norm'] = candidates['Campaign Name'].astype(str).str.strip().str.lower().str.replace(r'[^a-z0-9]', '', regex=True)
        candidates['_ag_norm'] = candidates['Ad Group Name'].astype(str).str.strip().str.lower().str.replace(r'[^a-z0-9]', '', regex=True)
        
        bulk = bulk.copy()
        bulk['_camp_norm'] = bulk['Campaign Name'].astype(str).str.strip().str.lower().str.replace(r'[^a-z0-9]', '', regex=True)
        bulk['_ag_norm'] = bulk['Ad Group Name'].astype(str).str.strip().str.lower().str.replace(r'[^a-z0-9]', '', regex=True)
        
        # For aggregated rows, match on Campaign + AdGroup only (don't match on targeting text)
        id_lookup = bulk.groupby(['_camp_norm', '_ag_norm']).agg({
            'KeywordId': 'first',
            'TargetingId': 'first'
        }).reset_index()
        ...
        # Clean up old ID columns if they exist
        candidates.drop(columns=['KeywordId_old', 'TargetingId_old', '_camp_norm', '_ag_norm'], inplace=True, errors='ignore')
    
    return candidates


def _optimize_bid(row: pd.Series, config: dict, alpha: float) -> Tuple[float, str]:
    """Calculate optimal bid using Current Bid as base (with CPC fallback and Caps)."""
    current_bid = float(row.get("Current Bid", 0) or 0)
    avg_cpc = float(row.get("CPC", 0) or 0)
    
    actual_roas = float(row.get("ROAS", 0) or 0)
    target_roas = float(row.get("Campaign_ROAS", config["TARGET_ROAS"]) or config["TARGET_ROAS"])
    clicks = int(row.get("Clicks", 0) or 0)
    
    # 1. Determine Base Bid
    # "Fallback: ONLY if Current Bid is missing, use Average CPC"
    ...
    if actual_roas > 0:
        ...
        if clicks > 10:
            adjustment = -config["MAX_BID_CHANGE"]
            reason = "Bleeder: 0 Sales"
        else:
            return base_bid, "Hold: Low Data" # Keep current bid
    
    # Cap adjustment size
    adjustment = max(-config["MAX_BID_CHANGE"], min(config["MAX_BID_CHANGE"], adjustment))
    
    # 3. Calculate New Bid
    new_bid = base_bid * (1 + adjustment)
    
    # 4. Safety Cap (2x Avg CPC)
    # "Ensure the new calculated bid does not exceed 2x Average CPC"
    if avg_cpc > 0:
        safety_cap = avg_cpc * 2.0
        if new_bid > safety_cap:
            new_bid = safety_cap
            reason += " (Capped at 2x CPC)"
...
def run_simulation(
    ...
    if not all_bids.empty:
        ...
        all_bids["CPC"] = pd.to_numeric(all_bids.get("Cost Per Click (CPC)", 0), errors="coerce").fillna(0)
        all_bids["New Bid"] = pd.to_numeric(all_bids.get("New Bid", 0), errors="coerce").fillna(0)
...
def generate_negatives_bulk(neg_kw: pd.DataFrame, neg_pt: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in pd.concat([neg_kw, neg_pt]).iterrows():
        ...
        if is_asin_neg:
            row_data["Product Targeting Expression"] = row.get("Term", "")
...
def generate_bids_bulk(bids_df: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
    ...
    for _, row in bids_df.iterrows():
        ...
        targeting = str(row.get("Targeting", "")).lower()
        # Check if it's a product targeting (ASIN or category)
        is_pt = (
            any(x in targeting for x in ["asin=", "category=", "close-match", "loose-match", "substitutes", "complements"]) or
            is_asin(targeting)  # Check if it's a raw ASIN (B0XXXXXXXXX format)
        )
        ...
        if is_pt:
            ...
            targeting_id = str(row.get("TargetingId", "")).strip()
            row_data["Product Targeting ID"] = targeting_id
...
class OptimizerModule(BaseFeature):
    ...
    def _run_analysis(self, df: pd.DataFrame):
        ...
        with st.spinner("Running Optimization Engine..."):
            ...
            direct_bids, agg_bids = calculate_bid_optimizations(df, self.config, harvested_terms)
    ...
    def _display_bids(self, direct_df: pd.DataFrame, agg_df: pd.DataFrame):
        """Display bid optimizations."""
        st.subheader("💰 Bid Optimizations")
        
        tab1, tab2 = st.tabs([f"Direct/Exact ({len(direct_df)})", f"Aggregated ({len(agg_df)})"])
        
        cols = ["Targeting", "Campaign Name", "Match Type", "Clicks", "ROAS", "Current Bid", "Cost Per Click (CPC)", "New Bid", "Reason"]
        
        with tab1:
            if not direct_df.empty:
                display_cols = [c for c in cols if c in direct_df.columns]
                st.dataframe(direct_df[display_cols], use_container_width=True)
            else:
                st.info("No direct bid optimizations")
        
        with tab2:
            st.markdown("*Aggregated to Match Type level (broad/phrase/category/auto targeting)*")
            if not agg_df.empty:
                display_cols = [c for c in cols if c in agg_df.columns]
                st.dataframe(agg_df[display_cols], use_container_width=True)
            else:
                st.info("No aggregated bid optimizations")
