...
def run_consolidated_optimizer():
    ...
    with st.spinner("Running Optimization Engine..."):
        ...
        corrected_term_set = set(harvest_df["Customer Search Term"].str.lower()) if not harvest_df.empty else set()
        neg_kw, neg_pt, your_products_review = identify_negative_candidates(df_prep, opt.config, harvest_df)
        direct_bids, agg_bids = calculate_bid_optimizations(df_prep, opt.config, corrected_term_set)
    ...
    with tabs[3]:
        opt._display_bids(direct_bids, agg_bids)
